...
def get_args():
    ...
    data_args_group.add_argument("--task_prefix_token", default="[NLU]", type=str)
...
def get_features(records, tokenizer, max_length, task_prefix):
    features = tokenizer(
        text=[
            task_prefix + standard
...
def main():
    ...
    additional_tokens_for_task = []
    tokenizer = AutoTokenizer.from_pretrained(args.pretrained_model_name_or_path)
    tokenizer.add_special_tokens({"additional_special_tokens": additional_tokens_for_task})

    # Same input format as T5 if no specified denoiser prefix token recognized
    if not args.task_prefix_token in tokenizer.get_vocab():
        print("task prefix token not included in tokenizer's vocab")
        args.task_prefix_token = "[dialect]"
        additional_tokens_for_task.append(args.task_prefix_token)

    if additional_tokens_for_task:
        tokenizer.add_special_tokens({"additional_special_tokens": additional_tokens_for_task})
    ...
    train_dataset = train_dataset.map(
        lambda records: get_features(records, tokenizer, args.max_sequence_length, args.task_prefix_token),
        batched=True,
